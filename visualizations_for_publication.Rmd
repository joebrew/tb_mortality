...
by_sex[by_sex$Value == 'Total TB' &
       ...
       c('IHME', 'WHO')] <- c(df$i_f_15plus_tb_nd +
                                df$i_f_15plus_htb_nd,
                              df$w_f_15plus_tb_nd +
                                df$w_f_15plus_htb_nd)
by_sex[by_sex$Value == 'Total TB' &
              by_sex$Region == 'Male',
       c('IHME', 'WHO')] <- c(df$i_m_15plus_tb_nd +
                                df$i_m_15plus_htb_nd,
                              df$w_m_15plus_tb_nd +
                                df$w_m_15plus_htb_nd)
...
by_sex[by_sex$Value == 'HIV+TB only' &
              by_sex$Region == 'Female',
       c('IHME', 'WHO')] <- c(df$i_f_15plus_htb_nd,
                              df$w_f_15plus_htb_nd)
by_sex[by_sex$Value == 'HIV+TB only' &
              by_sex$Region == 'Male',
       c('IHME', 'WHO')] <- c(df$i_m_15plus_htb_nd,
                              df$w_m_15plus_htb_nd)
...
x <- x[c(1:3, # total
         c(4:6, 25:27), # adults then children
...
df$cdr_ihme <- 
  # numerator: addition of new cases hiv and hiv+tb from ihme 
  df$ihme_incidence_both_allages_totaltb_number / 
  # denominator: (df$c_newinc + df$ret_nrel) 
  (df$c_newinc + df$ret_nrel)* 100
...
ggplot(data = df,
  ...
  labs(x = 'Region',
       ...
       title = 'Standardized difference by WHO region')
  # geom_violin()
```
...
ggplot(data = df,
  ...
  labs(x = 'Prevalence survey',
       y = 'Standardized difference',
       title = 'Association of prevalence survey and standardized difference')
  # geom_violin()
```
