---
title: "Visualizations for publications"
author: "joe@economicsofmalaria.com"
...
### Global differences by: HIV, age, sex, world region

```{r}
make_pretty(df_region)
```



2. Table on the different methods used by WHO and IHME

```{r}
make_pretty(data_frame(` ` = 'To be filled out by Alberto'))
```
...
Graph with rakings with highest absolute and standardized differences for IHME and WHO, (similar to slide 14 of current slide show, although we need to improve that one).
...
Map with logarithmic absolute differences or quadratic root? Similar to slide 38... with the legend + or negative depending on whether the association is towards WHO or IHME.
...
g1 <- 
  ...
  labs(x = 'Longitude',
       y = 'Latitude',
       title = 'Adjusted standardized difference',
       subtitle = '"adjusted_stand_dif"')
g1
```

Graph of correlation of both methods per country and… I don ´t know which is the most meaningful… Region? (slide 3-4)
...
## Analytical
6. Graph with 4 panels including scatter plots (or correlations) with association of standardized difference vs 
...
Visual additional material for the paper WHO-IHME
1. Interactive map providing all basic indicators when clicking the country
2. Table with the information of the interactive map.
