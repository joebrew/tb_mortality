...
Afternoon of Wednesday, April 18. All charts are IHME minus WHO.

```{r}
# Define function for getting ab and std diff
make_diff <- function(a, b, absolute = FALSE, standardize = TRUE){
  if(absolute){
    out <- a-b #abs(a-b)
  } else {
    out <- a-b/(a+b)/2
    if(standardize){
      out <- out / abs(max(c(max(out, na.rm = TRUE),
                         min(out, na.rm = TRUE)))) * 100
    }
  }
 return(out) 
}
...
### 1. HIV+ all ages absolute differences

```{r}
x <- df
x$val <- make_diff(a = df$i_both_all_htb_nd,
          b = df$w_both_all_htb_nd,
          absolute = TRUE)
plotter(x)
```

### 2. HIV+ all ages standardized adjusted differences
...
### 3. HIV- all ages absolute differences

```{r}
x <- df
x$val <- make_diff(a = df$i_both_all_tb_nd - df$i_both_all_htb_nd,
          b = df$w_both_all_tb_nd - df$w_both_all_htb_nd,
          absolute = TRUE)
plotter(x)
```


### 4. HIV- all ages standardized adjusted differences
...
### 5. Children absolute difference

```{r}
x <- df
x$val <- make_diff(a = df$i_both_014_tb_nd,
          b = df$w_both_014_tb_nd,
          absolute = TRUE)
plotter(x)
```

### 6. Children standardized adjusted difference
...
### 7. Adults standardized adjusted difference

```{r}
x <- df
x$val <- make_diff(a = df$i_both_all_tb_nd - df$i_both_014_tb_nd,
          b = df$w_both_all_tb_nd - df$w_both_014_tb_nd,
          absolute = TRUE)
plotter(x)
```
...
### 8. Adults standardized adjusted difference
